...
export function updateHealth() {
    try {
        ...
        if (healthText && window.gameState) {
            ...
            console.warn(`Could not update health display: healthText=${!!healthText}, gameState=${!!window.gameState}`);
        }
        
        if (window.gameState && window.gameState.health <= 0) {
            // Set game over state
            window.gameState.isGameOver = true;
            
            // Call the correct game over function with the scene parameter
            if (window.gameState.scene) {
                showGameOver(window.gameState.scene);
            } else {
                console.error('Scene not available for game over screen');
            }
        }
    } catch (error) {
...
export async function saveHighScore(playerName, score) {
    try {
        ...
        console.log(`High score saved for ${playerName}: ${score}`);
...
// Display high scores
async function displayHighScores(highScoresList) {
    try {
        const response = await fetch('/api/highscores');
        if (!response.ok) {
        ...
        if (highScores.length === 0) {
            highScoresList.setText('No high scores yet!');
            return;
        }
        
        // Format high scores as text with ranking
        ...
        highScores.forEach((score, index) => {
            ...
            // Format each score entry with rank, name, score, and date
            highScoresText += `${rank}. ${name}: ${scoreValue} (${formattedDate})\n`;
        });
        
        // Set text with proper formatting
        highScoresList.setText(highScoresText);
        highScoresList.setStyle({
            fontSize: '20px',
            fill: '#ffffff',
            fontFamily: 'Arial',
            align: 'center',
        ...
        highScoresList.setText('Error loading high scores');
    }
}
...
function fireBullet(scene) {
    if (scene.time.now > lastFired) {
        ...
        const bulletSpacing = 20; // Space between parallel bullets

        if (scene.isDoubleShot) {
            // Create two parallel bullets
            ...
            bullet1.body.setVelocityY(-bulletSpeed);
            bullet2.body.setVelocityY(-bulletSpeed);
...
export function activatePowerUp(type, scene) {
    ...
    if (!window.gameState) {
        ...
        window.gameState = { 
            ...
            currentLevel: 1, 
            isInvincible: false,
            scene: scene  // Store the scene reference
        };
    }
